...
public class fishCard extends cardWidget{
	...
	Actions action;
	Image img = Toolkit.getDefaultToolkit().getImage("/Users/asifmayilli/Desktop/Images/fishPink.png");
	BufferedImage bimg;
	int iw;
	int ih; 
	...
	public fishCard(int x, int y, int w, int h, int arcW, int arcH, double s, int fs){
		...
		try{
			BufferedImage bimg = ImageIO.read(new File("/Users/asifmayilli/Desktop/Images/fishPink.png"));
			iw = bimg.getWidth();
			ih = bimg.getHeight(); 
		}catch (IOException e) {
				    e.printStackTrace();
